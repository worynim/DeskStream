...
## [v3.1.0] - Interactive UI & Screen Flip System (2026-04-01)
### 🚀 주요 추가 사항
- **오디오 피드백 및 물리 버튼 통합**:
    - 7번 핀에 연결된 부저를 통해 **캘리브레이션 조작음** 기능을 구현했습니다. 버튼 터치 순간(50ms)과, 길게 눌러 진입 조건을 만족하는 순간(200ms) 각기 다른 햅틱 진동음이 발생합니다.
- **다이나믹 180도 화면 회전 (Screen Flip)**:
    - 1번 핀(버튼) 트리거를 통해 180도 화면 전환(Flip) 기능을 지원합니다.
    - **스마트 스크린 스왑(Swap)**: 디스플레이가 180도로 뒤집어질 때 표시되는 파형이 어긋나지 않도록, 512x64 캔버스의 잘라내기 할당 로직을 `1-2-3-4`에서 `4-3-2-1` 슬롯으로 능동 교체하여 매끄러운 시각화 연결성을 유지합니다.
- **비휘발성 메모리(NVS) 저장소 도입**:
    - 변경된 Flip 설정은 즉시 내부 플래시 메모리(`Preferences` 객체의 `viz_set` 영역)에 저장되며, 펌웨어 초기 부팅 시 읽어와 이전 하드웨어 상태를 그대로 복원합니다.
