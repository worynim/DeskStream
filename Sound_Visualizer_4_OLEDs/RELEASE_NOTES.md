...
- **오실로스코프 모드(Oscilloscope Mode) 추가**:
    - 스펙트럼 분석뿐만 아니라 실제 오디오 파형을 시간 영역에서 실시간으로 관찰할 수 있는 모드를 구현했습니다.
    - 자동 스케일링(Auto-Scaling) 기능을 통해 입력 음량에 맞춰 파형 크기가 OLED 화면에 최적화되어 표시됩니다.
- **다중 시각화 모드 전환 (BTN 4)**:
    - 4번 버튼(GPIO 9)의 숏프레스 동작을 시각화 모드 전환 기능으로 할당했습니다. 이제 스펙트럼 막대와 오실로스코프 파형을 필요에 따라 전환하며 즐길 수 있습니다.
- **대화형 버튼 도움말 화면 (BTN 2)**:
    - 2번 버튼(GPIO 4)을 1초 이상 길게 누르면 현재 활성화된 버튼들의 기능 안내 페이지가 OLED에 표시됩니다. (토글 방식)
- **고도화된 버튼 핸들링 아키텍처**:
    - 모든 물리 버튼에 숏프레스(Short)와 롱프레스(Long)를 명확히 구분하는 비차단 스캔 로직을 적용하여, 시각화 중단 없이 즉각적인 사용자 피드백을 제공합니다.

## [v3.2.0] - Non-blocking Audio & Logic Refinement (2026-04-02)
### 🚀 주요 개선 사항
- **비차단(Non-blocking) 부저 시스템 도입**:
    - `beep()` 함수의 `delay()` 블로킹으로 인해 발생하던 ADC DMA 버퍼 오버플로우와 파형 끊김 문제를 해결했습니다.
    - `millis()` 타이머 기반의 `beepAsync()` 및 `updateTone()` 시스템을 구축하여 사운드 피드백 중에도 중단 없는 실시간 시각화를 보장합니다.
- **AGC(자동 이득 제어) 정밀도 개선**:
    - AGC 계산 기준을 'Raw FFT 데이터'에서 '최종 렌더링 밴드 진폭(노이즈 차감 후)'으로 변경했습니다.
    - 로그(Log) 및 거듭제곱(Power) 매핑 모드에서 에너지 합산(Sum Pooling) 결과가 반영된 실제 화면 크기에 맞춰 감도가 더 정확하게 조절됩니다.
- **코드 안정성 및 빌드 최적화**:
    - **워치독 안전성**: 캘리브레이션 진입 대기 루프를 `delay(10)`에서 `vTaskDelay`로 교체하여 시스템 안정성을 높였습니다.
    - **상수 하드코딩 제거**: FFT 초기화 시 사용되던 매직 넘버(`1024`)를 `SAMPLES` 상수로 대체했습니다.
- **문서 및 설정 오류 교정**:
    - `config.h` 내의 잘못된 주석들(FPS 70 vs 40, 샘플링 40kHz vs 10kHz 등)을 실제 동작 값에 맞게 전면 수정했습니다.
    - README의 MCU 사양 오기(ESP32-C3의 FPU 유무)를 "FPU 없음"으로 바로잡았습니다.
    - `FLIP_BUTTON_PIN`을 `config.h`로 통합 관리하도록 개선했습니다.
