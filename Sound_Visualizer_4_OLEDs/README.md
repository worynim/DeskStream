...
이 프로젝트는 2개의 버튼을 통해 실시간 설정 및 캘리브레이션을 지원하며, 패시브 부저(GPIO 7)를 이용한 청각적 피드백을 제공합니다.

*   **BTN 1 (GPIO 1)**:
    *   **짧게 누름**: 화면 180도 회전(Flip) 토글.
    *   **스마트 스왑**: 화면이 뒤집힐 때 OLED 표시 순서도 자동 스왑(`1-2-3-4` ↔ `4-3-2-1`)되어 파형의 연속성을 유지합니다.
    *   설정 상태는 비휘발성 메모리(NVS)에 자동 저장되어 재부팅 시 복원됩니다.
*   **BTN 4 (GPIO 9)**:
    *   **짧게 누름**: 상태 확인용 비프음 발생.
    *   **길게 누름 (1초 이상)**: 노이즈 캘리브레이션 시작. (조용한 환경에서 캘리브레이션 하세요.)
