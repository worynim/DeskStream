...
### 7. [Screen_To_OLEDs: 실시간 화면 스트리머 (v3.1.0)](./Screen_To_OLEDs/)
- **기능**: Mac의 화면 일부를 실시간 캡처하여 DeskStream 4-OLED 매트릭스(512x64)에 무선 전송.
- **특징**: 
    - **지능형 캡처 엔진**: 'Full' 설정 시 핸들이 위치한 모니터를 자동 인식하여 전체 너비를 확보하는 스마트 리사이징.
    - **전문가용 디더링**: Atkinson, Stucki, Sierra 등 6종의 고성능 이진화 알고리즘 탑재 (Numba JIT 가속).
    - **실시간 제어 파이프라인**: 스트리밍 중단 없이 레이아웃(가로/세로), 캡처 너비, 오버레이 설정을 즉각 변경 가능.
- **버튼 동작**: 버튼 2를 눌러 밝기를 조절하고, 버튼 1을 롱프레스하여 화면을 180도 회전(플립)합니다.

### 8. [Hangeul_Clock: 고화질 폰트 한글 시계 (v3.9.0)](./Hangeul_Clock/)
- **기능**: NTP 동기화 기반의 고화질 한글 시계 및 하이엔드 폰트 렌더링 시스템.
- **특징**: 
    - **Font Studio v2**: TTF 폰트를 웹에서 직접 낱자(Character) 비트맵으로 변환하여 실시간 적용.
    - **지능형 레이아웃**: 오전/오후 정중앙 정렬 및 시/분/초 단위 우측 고정 + 숫자 개별 중앙 정렬 엔진 탑재.
    - **초고성능 아키텍처 (New)**: H/CPP 분리 모듈화, **Hardware HAL(I2C_Platform) 도입**, 동적 비트맵 캐시 및 Flash 기반 Web UI로 **RAM 사용량을 최적화**하고 유지보수성을 극대화했습니다.
    - **메모리 최적화**: 디스플레이에 필요한 38개의 고유 낱자로 압축 관리하여 LittleFS 효율 극대화.
