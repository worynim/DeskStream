...
### 8. [Hangeul_Clock: 고화질 폰트 한글 시계 (v2.0.0)](./Hangeul_Clock/)
- **기능**: NTP 동기화 기반의 고화질 한글 시계 및 하이엔드 폰트 렌더링 시스템.
- **특징**: 
    - **전문가급 안정성 (New)**: H/CPP 분리 모듈화, **하드웨어 자가 치유(I2C Recovery) 도입**, 비차단(Non-blocking) 애니메이션 상태 머신 구조로 전면 개편되었습니다.
    - **Flat Buffer 메모리 최적화**: 힙 파편화를 원천 차단하는 **Flat Buffer** 방식의 단일 메모리 할당으로 RAM 사용량과 구동 신뢰성을 극대화했습니다.
