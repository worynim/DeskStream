...
void startWebServer() {
    server.on("/", HTTP_GET, []() { server.send(200, "text/html", font_studio_html); });
    server.on("/upload", HTTP_POST, []() { server.send(200, "text/plain", "OK"); }, []() {
        HTTPUpload& upload = server.upload();
        static File fsUploadFile;
        if (upload.status == UPLOAD_FILE_START) fsUploadFile = LittleFS.open("/" + upload.filename, "w");
    ...
    server.on("/api/refresh_cache", HTTP_POST, []() {
        ...
        extern unsigned long forceUpdateTrigger; forceUpdateTrigger = 1;
        server.send(200, "text/plain", "OK");
    });
    server.on("/api/config", HTTP_GET, []() {
        String json = "{\"anim_mode\":" + String((int)display.anim_mode) + ",\"display_mode\":" + String((int)display.display_mode) + 
                     ",\"hour_format\":" + String((int)display.hour_format) + ",\"chime_enabled\":" + String(display.chime_enabled ? "true":"false") + 
                     ",\"font_name\":\"" + display.font_name + "\",\"is_flipped\":" + String(display.is_flipped ? "true":"false") + "}";
    ...
    server.on("/api/config", HTTP_POST, []() {
        if (server.hasArg("plain")) {
            ...
            auto parseBool = [&](String key) { return body.indexOf("\"" + key + "\":true") != -1; };
            auto parseVal = [&](String key) -> int {
                int pos = body.indexOf("\"" + key + "\":");
                if (pos == -1) return -1;
                int start = pos + key.length() + 2;
                while (start < body.length() && !isDigit(body[start])) start++;
                String v = ""; while (start < body.length() && isDigit(body[start])) v += body[start++];
                return v.length() > 0 ? v.toInt() : -1;
            };
            int am = parseVal("anim_mode"); if(am != -1) display.setAnimMode(am);
            int dm = parseVal("display_mode"); if(dm != -1) display.setDisplayMode(dm);
            int hf = parseVal("hour_format"); if(hf != -1) display.setHourFormat(hf);
            if (body.indexOf("\"chime_enabled\"") != -1) display.setChime(parseBool("chime_enabled"));
            if (body.indexOf("\"is_flipped\"") != -1) display.setFlipDisplay(parseBool("is_flipped"));
            int fnS = body.indexOf("\"font_name\":\"");
            if (fnS != -1) { fnS += 13; int fnE = body.indexOf("\"", fnS); if (fnE != -1) display.setFontName(body.substring(fnS, fnE)); }
            forceUpdateTrigger = 1; server.send(200, "text/plain", "OK");
        }
    });
    server.begin();
